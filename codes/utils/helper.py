...
def load_point_trajectory_grouped(file_path, box_size, round_dp=None):
    """
    Reads comma-delimited lines <t>,<x>,<y> (unsorted),
    sorts them by t, then groups every entry with the same t (optionally rounded)
    into one frame: [t, (x0,y0,x1,y1), ...].
    If round_dp is not None, timestamps are rounded to that many decimal places
    before grouping.
    """
    half = box_size / 2.0
    # read & parse
    ...
    for t, x, y in records:
        if current_t is None or t != current_t:
            ...
            if frame_boxes:
                trajectories.append([current_t] + frame_boxes)
            frame_boxes = []
            current_t = t
        # wrap point into a box
        x0, y0 = x - half, y - half
        x1, y1 = x + half, y + half
